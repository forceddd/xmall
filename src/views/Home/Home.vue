<template>
  <div class="home">
    ...
    <div v-for="item in homeList" :key="item.id">
      <!-- type为1时 显示活动板块内容 -->
      <div v-if="item.type===1" class="activity-panel">
        <el-row>
      ...
      <section class="w mt30 clearfix" v-if="item.type===2">
        <my-shelf :title="item.name">
          <!-- 具名插槽 -->
          <div slot="content" class="hot">
            <mall-goods v-for="i in item.panelContents" :key="i.id" :goods="i"></mall-goods>
          </div>
        </my-shelf>
      </section>
...
<script>
//引入shelf组件
import MyShelf from "@/components/Shelf.vue";
...
export default {
  ...
  async created() {
    try {
      const res = await this.$axios.get("/api/goods/home");
      // console.log(res);
      let data = res.data;
      if (data.code == 200) {
        let banner = data.result.find(i => i.type === 0);
        // console.log(banner);
